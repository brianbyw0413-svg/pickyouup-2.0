...
// 偵測是否為林口區地址
const isLinkouAddress = (address: string): boolean => {
...
// 取得報價（林口區優先）
const getPricing = (carType: string, mode: string, address: string) => {
...
const CREDIT_CARD_LINKS = {
  800: 'https://api.payuni.com.tw/api/uop/receive_info/2/1/U03424091/xxx',
  900: 'https://api.payuni.com.tw/api/uop/receive_info/2/1/U03424091/xxx',
...
export default function App() {
  ...
  // ── 價格 ──
  // 根據模式和地址取得報價（林口區特別價格）
  const currentAddress = mode === 'dropoff' ? dropoffForm.address : mode === 'pickup' ? pickupForm.address : dropoffForm.address;
          ...
          <Card>
            ...
            {(mode === 'dropoff' || mode === 'both') && (
              <div style={{ marginBottom: mode === 'both' ? 20 : 0 }}>
                ...
                {isLinkouAddress(dropoffForm.address) && <div style={{ color: S.gold, fontSize: 12, marginTop: 4 }}>🌟 林口區特別優惠價</div>}
            ...
            {(mode === 'pickup' || mode === 'both') && (
              <div>
                ...
                <Row label="車型" value={getCarLabel(carType)} />
                {isLinkouAddress(pickupForm.address) && <div style={{ color: S.gold, fontSize: 12, marginTop: 4 }}>🌟 林口區特別優惠價</div>}
