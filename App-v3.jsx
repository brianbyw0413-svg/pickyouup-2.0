  ...
  const lookupFlight = async (flightNumber, date, formMode) => {
    ...
    try {
      const res = await fetch(
        `${TDX_PROXY_URL}?flight=${encodeURIComponent(flightNumber.trim().toUpperCase())}`
      );
      ...
      // 查無航班
      if (!data.found) {
        alert(data.error || '查無此航班，請確認航班編號是否正確（僅支援當日航班）');
      ...
      if (formMode === 'dropoff') {
        ...
        const depTime = extractTime(data.ScheduleDepartureTime);
        if (depTime) {
